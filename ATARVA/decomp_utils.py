...
def motif_decomposition(sequence, motif_size, motif):

    best_motif_size = min(motif_size , shift_and_match(sequence, motif_size)) 
    primary_motif, secondary_motif = get_most_frequent_motif(sequence, best_motif_size, motif)
    ...
    prefix = sequence[:positions[0]]
    if len(prefix) >= motif_size * 2:
        secondary_motif_size = shift_and_match(prefix, motif_size)
        secondary_decomposition = motif_decomposition(prefix, secondary_motif_size, '')
        decomposed_parts.append(secondary_decomposition)
    elif prefix:
        decomposed_parts.append(prefix)
